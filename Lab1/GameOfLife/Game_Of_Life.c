...
int main (int argc, char * argv[]) {
	int n_workers;			//Number of threads for openmp
	...
	if ( argc != 4 ) {
		fprintf(stderr, "Usage: ./exec ArraySize TimeSteps NumberOfWorkers\n");
		exit(-1);
	}
	else {
		N = atoi(argv[1]);
	...
	for ( t = 0 ; t < T ; t++ ) {
        #pragma omp parallel for num_threads(n_workers), default(shared), private(i,j,nbrs)
		for ( i = 1 ; i < N-1 ; i++ )
			for ( j = 1 ; j < N-1 ; j++ ) {
