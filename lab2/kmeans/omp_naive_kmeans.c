            ...
            double * clusters)         /* out: [numClusters][numCoords] */
{
    ...
    do {
        ...
        #pragma omp parallel for default(none) shared(newClusterSize, delta, newClusters, numObjs, membership, objects, clusters) firstprivate(numCoords, numClusters) private(i, index, j)
        for (i=0; i<numObjs; i++) {
