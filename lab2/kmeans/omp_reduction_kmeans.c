            ...
            double * clusters)         /* out: [numClusters][numCoords] */
{
    ...
    #pragma omp parallel for default(none) private(k) shared(nthreads, local_newClusterSize, local_newClusters, numClusters, numCoords)
    ...
    do {
         ...
         */
        #pragma omp parallel for default(none) private(thread, i, j) firstprivate(local_newClusterSize, local_newClusters) shared(numClusters, numCoords, nthreads)
        for (thread = 0; thread < nthreads; thread++){
            for (i = 0; i < numClusters; i++){
                local_newClusterSize[thread][i] = 0;
                for (j = 0; j < numCoords; j++){
                    local_newClusters[thread][i*numCoords + j] = 0;
                }
            }
        }

        #pragma omp parallel for default(none) firstprivate(local_newClusters, local_newClusterSize, numCoords, numClusters) private(i, j, index, tid) shared(newClusterSize, newClusters, numObjs, membership, objects, clusters, nthreads) reduction(+:delta)
        for (i=0; i<numObjs; i++)
        {
        ...
        for (thread = 0; thread < nthreads; thread++){
            for (i = 0; i < numClusters; i++){
                        newClusterSize[i] += local_newClusterSize[thread][i];
                for (j = 0; j < numCoords; j++){
                    newClusters[i*numCoords + j] += local_newClusters[thread][i*numCoords + j];
                }
            }
        }
